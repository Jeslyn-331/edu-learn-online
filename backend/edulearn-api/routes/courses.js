...
// ============================================================
router.post('/', verifyToken, isInstructor, upload.single('image'), async (req, res) => {
    try {
        const { title, description, price, is_published, duration } = req.body;
        ...
        if (req.file) {
            const s3Key = `images/courses/${Date.now()}-${req.file.originalname}`;
            
            finalImageUrl = await uploadToS3(req.file.path, s3Key, req.file.mimetype);
        ...
        const [result] = await pool.query(
            `INSERT INTO courses (title, description, price, image_url, instructor_id, is_published, duration) 
             VALUES (?, ?, ?, ?, ?, ?, ?)`,
            [title, description || '', coursePrice, finalImageUrl, req.user.user_id, is_published || false, courseDuration]
        );
...
// ============================================================
router.put('/:id', verifyToken, isInstructor, upload.single('image'), async (req, res) => {
    try {
        const courseId = req.params.id;
        ...
        if (image_url !== undefined) { updates.push('image_url = ?'); params.push(finalImageUrl); }
